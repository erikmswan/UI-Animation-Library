...
const ANIM = (function() {
    ...
    function getTimingDelay(el) {
        const timingDelayAttr = parseInt($(el).attr('timing-delay'));
        const timingDelay     = isNaN(timingDelayAttr) ? null : timingDelayAttr;
        return timingDelay;
    }

    // Utility function to get the timing increment from the 'timing-increment' attribute
    function getTimingIncrement(el) {
        const timingIncrementAttr = parseInt($(el).attr('timing-increment'));
        const timingIncrement     = isNaN(timingIncrementAttr) ? null : timingIncrementAttr;
    ...
    function setChildTiming(el, start, increment, type) {
        let timingDelay     = start;
        const realIncrement = increment === undefined ? 0 : increment;

        $(el).each(function() {
            if (type === 'transition')
                $(this).css('transition-delay', `${timingDelay}ms`);
            if (type === 'animation')
                $(this).css('animation-delay', `${timingDelay}ms`);
    ...
    function letterFadeIn(selector, fadeInClass, start, increment) {
        ...
        $(selector).each(function() {
            const split = $(this).html().match(/[a-zA-Z\d&;<\/="\->\s\n]/g);
            let htmlEntity = false;
            let htmlEntityContainer;
            const mappedSplit = split.map(function(el) {
        ...
        const timingDelay     = start ? start : getTimingDelay(selector);
        const timingIncrement = increment ? increment : getTimingIncrement(selector);
